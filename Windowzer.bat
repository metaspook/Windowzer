...
echo ^|  5. Remove Windows watermark.
echo ^|  6. Restart Windows Explorer.
echo ^|  7. Run CMD as Administrator.
echo ^|  8. Read Notes.
echo ^|  0. Exit.
echo.
...
if "%CMVAR%"=="5" (
	REM ; Remove Windows  watermark.
	set RRSTR1=
	>nul 2>&1 reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform\Activation" /t REG_DWORD /d "1" /f
	>nul 2>&1 reg add "HKCU\Control Panel\Desktop" /v "PaintDesktopVersion" /t REG_DWORD /d "0" /f
	>nul 2>&1 reg add "HKLM\SYSTEM\CurrentControlSet\Services\svsvc" /v "Start" /t REG_DWORD /d "4" /f
	>nul 2>&1 reg add "HKLM\SYSTEM\ControlSet001\Services\svsvc" /v "Start" /t REG_DWORD /d "4" /f
	(taskkill /F /IM explorer.exe && explorer.exe) >nul 2>&1
	set RRSTR1=[DONE] Windows watermark removed.
	goto:REBOOTREQ
)
if "%CMVAR%"=="6" (taskkill /F /IM explorer.exe && explorer.exe) >nul 2>&1
if "%CMVAR%"=="7" start /i %windir%\System32\cmd.exe
if "%CMVAR%"=="8" (
...
echo.
echo [NOTE!] This Option don't need to re-apply 
echo         again untill you face any issue.   
echo.
