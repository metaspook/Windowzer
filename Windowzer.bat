...
echo ^|  5. Remove "Activate Windows" watermark.
...
if "%CMVAR%"=="5" (
	REM ; Remove "Activate Windows" watermark.
	set RRSTR1=
	>nul 2>&1 reg add "HKCU\Control Panel\Desktop" /v "PaintDesktopVersion" /t REG_SZ /d "0" /f
	set RRSTR1=[DONE] "Activate Windows" watermark Removed.
	goto:REBOOTREQ
)
